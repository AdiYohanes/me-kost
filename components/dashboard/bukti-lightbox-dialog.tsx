...
export function BuktiLightboxDialog({
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="max-w-md p-4 sm:p-6 overflow-hidden max-h-[90vh] flex flex-col">
        ...
        <div className="space-y-3 overflow-y-auto pr-1">
          ...
          <div className="p-3 bg-slate-50 border border-slate-200/80 rounded-xl space-y-2">
            ...
            {buktiPembayaran.catatanPenghuni && (
              <div className="p-2 bg-white rounded-lg border border-slate-100 text-[11px] text-slate-600 flex items-start gap-1.5">
          ...
          <div className="pt-3 border-t border-slate-100 flex items-center gap-2 shrink-0">
            {onReject && (
              <Button
                ...
                variant="outline"
                onClick={onReject}
                className="flex-1 h-9 text-xs font-semibold text-rose-600 border-rose-200 hover:bg-rose-50 hover:border-rose-300 gap-1.5"
              >
            ...
            {onApprove && (
              <Button
                type="button"
                onClick={onApprove}
                className="flex-1 h-9 text-xs font-bold bg-emerald-600 hover:bg-emerald-700 text-white gap-1.5"
              >
